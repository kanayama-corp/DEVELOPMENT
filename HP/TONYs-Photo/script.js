...
function initHeroTitleAnimation() {
    ...
    const textLines = animatedText.querySelectorAll('.text-line');
    ...
    textLines.forEach((line, lineIndex) => {
        ...
        text.split('').forEach((char, charIndex) => {
            ...
            // Only animate title section (lines 1-2)
            if (lineIndex < 2) {
                // Title section (lines 1-2): animate with delay
                const charDelay = charIndex + lineIndex * 20;
                span.style.setProperty('--char-index', charDelay);
            } else {
                // Description section (lines 3-4): no animation, show immediately
    ...
    setTimeout(() => {
        animatedText.classList.add('is-active');
    }, 1000);
}
...
// Hero image slideshow - simple fade transition
...
function switchHeroImage() {
    console.log(`Switching from image ${currentImageIndex + 1} to image ${((currentImageIndex + 1) % heroImages.length) + 1}`);
    ...
    // すべての画像をフェードアウト
    heroImages.forEach((imgId, index) => {
    ...
    if (nextImg) {
        ...
        console.log(`Next image ${currentImageIndex + 1} (${heroImages[currentImageIndex]}) faded in`);
    }
}
...
// 初期化時にすべての画像を準備し、最初の画像を表示
document.addEventListener('DOMContentLoaded', function() {
    // すべての画像要素を確認し、存在することを保証
    heroImages.forEach((imgId, index) => {
        const img = document.getElementById(imgId);
        if (img) {
            // すべての画像を非表示状態で初期化
            ...
            // 画像の読み込み完了を確認
            if (img.complete) {
                console.log(`Image ${index + 1} (${imgId}) loaded successfully`);
            } else {
                img.addEventListener('load', function() {
                    console.log(`Image ${index + 1} (${imgId}) loaded successfully`);
                });
                img.addEventListener('error', function() {
                    console.log(`Image ${index + 1} (${imgId}) failed to load, using fallback`);
                });
            }
        } else {
            console.error(`Image element ${imgId} not found in DOM`);
        }
    });
    ...
    // デバッグ用：すべての画像要素の状態をログ出力
    console.log('Hero images initialization:', {
        totalImages: heroImages.length,
        imageElements: heroImages.map(id => {
